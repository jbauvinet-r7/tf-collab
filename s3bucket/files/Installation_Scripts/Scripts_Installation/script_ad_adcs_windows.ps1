...
if ((Get-WindowsFeature ADCS-Cert-Authority).installed -ne "True")
{
    Write-Log -Message "Windows feature (ADCS) is not installed and will be installed."
    Install-WindowsFeature -Name ADCS-Cert-Authority -IncludeManagementTools
    Install-AdcsCertificationAuthority -Force
    Install-ADcsCertificationAuthority -Credential $mycreds -CAType StandaloneRootCA -CACommonName "domain-Host1-CA-1" -CADistinguishedNameSuffix "DC=$DomainNetbiosName,DC=$DomainExt" -CryptoProviderName "RSA#Microsoft Software Key Storage Provider" -KeyLength 2048 -HashAlgorithmName SHA1 -ValidityPeriod Years -ValidityPeriodUnits 3 -DatabaseDirectory "C:\windows\system32\certLog" -LogDirectory "c:\windows\system32\CertLog" -Force 
} else
{
    Write-Log -Message "Windows feature (ADCS) is already installed."
} 