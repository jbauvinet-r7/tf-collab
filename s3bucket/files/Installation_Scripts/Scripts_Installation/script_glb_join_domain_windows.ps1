...
Write-Log -Message "Checking Domain"
if ((Get-WmiObject -Class Win32_ComputerSystem).PartOfDomain -ne 'True')
{
    Write-Log -Message "Not In domain, joining domain"
    ...
    Write-Log -Message "Domain joined, reboot in progress"
} else {
    Write-Log -Message "Already In domain"
}