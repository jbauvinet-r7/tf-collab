...
echo "$timestamp - Installing Network Sensor"
mkdir /opt/temp/
cd /opt/temp/
aws s3 cp s3://$Bucket_Name/Sensor_Scripts/sensor_installer.sh /opt/temp/
chmod +x /opt/temp/sensor_installer.sh
./sensor_installer.sh install_start --token $Token
echo "$timestamp - Network Sensor Installed"