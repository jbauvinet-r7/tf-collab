...
if [[ "$Routing_Type" == "pfsense" ]]; then
  ...
  echo "$timestamp - Static gateway configured for $interface1. IP: $Instance_IP1, Gateway: $Instance_GW"
  ...
  #   # sudo netplan try (optional, for verification)
  # fi
  # echo "Static gateway configured for $interface2. IP: $Instance_IP2, Gateway: $Instance_GW"
  echo "Network Settings Changed"
else
  echo "Routing Type AWS, no post networking needed"
fi